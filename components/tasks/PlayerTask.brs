...
sub PlayContentWithAds()
    ...
    while keepPlay
        ...
        if itemsCount - 1 > index
            ...
            ' Create an object to pass into the callback function for RAF events in order to store bookmark positions
            callbackObjectMetaData = {
                bookmarks: bookmarks
                item: item
            }
            ' Set RAF's tracking callback function to store bookmarks on content position updates
            RAF.SetTrackingCallback(handleRafEvent, callbackObjectMetaData)
...
' The callback function for when RAF sends tracking events. In this case, we want to store bookmark positions when
' RAF reports a position update. We will store a bookmark position every 10 seconds.
function handleRafEvent(obj = invalid as Object, eventType = invalid as Object, ctx = invalid as Object)
    if eventType <> invalid and type(eventType) = "roString" and eventType = "ContentPosition"
        if ctx <> invalid and type(ctx) = "roAssociativeArray"
            if ctx.contentPos MOD 10 = 0 'Store a bookmark position every 10 seconds
                obj.bookmarks.UpdateBookmarkForVideo(obj.item, ctx.contentPos)
            end if
        end if
    end if
end function